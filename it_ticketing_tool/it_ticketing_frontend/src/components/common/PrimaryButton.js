...
const PrimaryButton = ({ children, onClick, loading, Icon, type = 'button', disabled, className = '' }) => (
    <button
        type={type}
        onClick={onClick}
        className={`bg-[#e85c34] text-white py-2 px-3 rounded-md font-bold text-sm hover:bg-[#e85c34]/90 focus:outline-none focus:ring-2 focus:ring-[#e85c34]/30 transition duration-300 flex items-center justify-center space-x-2 disabled:opacity-60 disabled:cursor-not-allowed shadow-md hover:shadow-lg transform hover:scale-[1.01] ${className}`}
